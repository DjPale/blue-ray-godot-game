...
func addImageNodes():
	...
	if dock.get_node(mainGuiPath+"HBoxImageFrame/frame1").is_editable():
		for _imagePath in imagesPath:
			...
			var _baseName = dock.get_node(mainGuiPath+"HBoxImage/VBoxImage/name/lblName").get_text()
			for _frame in range(_startFrame,_endFrame):
				var _imageName = _baseName+str(_frame)
				var _newSpriteNode
				if !_root.has_node(_imageName):
					...
					var x = int(_frame)%int(hFrames)
					var y = int(_frame/hFrames)
					_newSpriteNode.set_pos(Vector2(x*tileSize,y*tileSize))
